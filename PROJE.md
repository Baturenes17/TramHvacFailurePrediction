...
- **LightGBM** (`LGBMClassifier`). Bu sürümde tek model (gerekçe: 67k satır, tablo tipi, %20
  dengesiz veri LightGBM'in tam sahası; ensemble belgeye göre yalnız ~0.03 AUC ekliyor → azalan verim).
- `build_model()` ileride başka modeller eklemeye açık yazıldı.
- **Sınıf dengesizliği:** `scale_pos_weight = (negatif sayısı / pozitif sayısı)`.
- **Varsayılan hiperparametreler:** `n_estimators=600`, `learning_rate=0.03`, `num_leaves=31`,
  `subsample=0.8`, `colsample_bytree=0.8`, `reg_lambda=1.0`, `random_state=42`.
- **Early stopping:** Eğitim, validation seti üzerinde `auc` ile erken durdurma yapar
  (`EARLY_STOPPING_ROUNDS=50`); seçilen `best_iteration` konsola yazılır ve final (tüm-veri) model
  bu ağaç sayısıyla yeniden eğitilir.
...
### Eşik seçimi (iki mod)
- **`alarm` (varsayılan):** **her split kendi** skorlarının en yüksek `--alarm-rate` (vars. %40)
  dilimini pozitif yapar (val ve test için ayrı eşik). Base-rate kaymasına dayanıklı ("her dönem
  en riskli %k aracı incele"). Konsolda ayrıca referans olarak val üzerinden `op(recall≥0.80)` ve
  `F1-opt` eşikleri raporlanır.
...
### Gözlemlenen sonuçlar (varsayılan çalıştırma)
- **Test ROC-AUC ≈ 0.73**, PR-AUC ≈ 0.30 (base-rate ~%16'nın üstünde → model anlamlı sinyal taşıyor).
- **Önemli not — zamansal kararsızlık:** Validation penceresinde ROC-AUC ~0.54'e kadar düşüyor;
  yani arıza ile özellikler arasındaki ilişki dönemden döneme belirgin kayıyor. Bunun bir yan etkisi:
  early stopping bu "zor" validation penceresinde çok erken duruyor (`best_iteration` küçük, ör. ~9),
  dolayısıyla final model **sığ/underfit** kalıyor ve güncel olasılıklar dar bir bantta kümeleniyor.
  Bu, modelden çok **verinin non-stasyoner** yapısının sonucu ve `PROJE_TANITIM.md`'deki "base-rate
  kayması" notuyla tutarlı. Pratik öneri: alarm-oranı modu + düzenli yeniden eğitim; ileride arıza
  geçmişi özellikleri eklemek validation'ı toparlayabilir (bkz. bölüm 9).
...
# Sabit eşik modu (recall>=0.80 kısıtı)
